...
import {
    QueryExpand, QueryHierarchyItem, QueryResultType, Wiql, WorkItem,
...
export class WorkItemTrackingService {
    private _witApi: IWorkItemTrackingApi | undefined;

    constructor(context: TeamServerContext) {
        if (context.RepoInfo) {
            new WebApi(context.RepoInfo.CollectionUrl!, CredentialManager.GetCredentialHandler()).getWorkItemTrackingApi().then((item) => {
                this._witApi = item;
            });
        }
    }
    ...
    public async CreateWorkItem(context: TeamServerContext, itemType: string, taskTitle: string): Promise<WorkItem | undefined> {
        const newWorkItem = [{ op: "add", path: "/fields/" + WorkItemFields.Title, value: taskTitle }];
        if (this._witApi && context.RepoInfo) {
    ...
    public async GetWorkItems(teamProject: string, wiql: string | undefined): Promise<SimpleWorkItem[]> {
    ...
    public async GetWorkItemTypes(teamProject: string): Promise<WorkItemType[]> {
        const types: WorkItemType[] | undefined = await this._witApi?.getWorkItemTypes(teamProject);
        const workItemTypes: WorkItemType[] = [];
        const hiddenTypes: WorkItemTypeReference[] = [];
        types?.forEach((type) => {
            workItemTypes.push(type);
        });
        const category: WorkItemTypeCategory | undefined = await this._witApi?.getWorkItemTypeCategory(teamProject, "Microsoft.HiddenCategory");
        if (category) {
    ...
    public async GetWorkItemById(id: string): Promise<SimpleWorkItem> {
        const workItem: WorkItem | undefined = await this._witApi?.getWorkItem(parseInt(id), [WorkItemFields.Id, WorkItemFields.Title]);
    ...
    private async execWorkItemQuery(teamProject: string, wiql: Wiql): Promise<SimpleWorkItem[]> {
        ...
        const queryResult: WorkItemQueryResult | undefined = await this._witApi?.queryByWiql(wiql, teamContext);
        const results: SimpleWorkItem[] = [];
        let workItemIds: number[] = [];
        if (queryResult) {
            if (queryResult.queryResultType === QueryResultType.WorkItem) {
                workItemIds = queryResult.workItems!.map(function (w) { return (w.id ? w.id : 0); });
            } else if (queryResult.queryResultType === QueryResultType.WorkItemLink) {
                workItemIds = queryResult.workItemRelations!.map(function (w) { return (w.target && w.target.id ? w.target.id : 0); });
            }
        ...
        if (workItemIds && workItemIds.length >= WorkItemTrackingService.MaxResults) {
        ...
        for (let index: number = 0; index < workItemIds.length; index++) {
            const item: WorkItem | undefined = workItems.find((i) => i.id === workItemIds[index]);
            if (item && item.id) {
    ...
    public async GetQueryResultCount(teamProject: string, wiql: string | undefined): Promise<number> {
        //Querying WIT requires a TeamContext
        const teamContext: TeamContext = {
            projectId: undefined,
            project: teamProject,

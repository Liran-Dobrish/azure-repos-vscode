...
export class WitClient extends BaseClient {
    ...
    private _myQueriesFolder: string | undefined;
    ...
    public async CreateNewWorkItem(taskTitle: string): Promise<void> {
        try {
            ...
            const selectedType: BaseQuickPickItem | undefined = await window.showQuickPick(this.getWorkItemTypes(), { matchOnDescription: true, placeHolder: Strings.ChooseWorkItemType });
            if (selectedType) {
                Telemetry.SendEvent(TelemetryEvents.OpenNewWorkItem);
    ...
    public async ShowMyWorkItemQueries(): Promise<void> {
        try {
            ...
            const query: WorkItemQueryQuickPickItem | undefined = await window.showQuickPick(this.getMyWorkItemQueries(), { matchOnDescription: false, placeHolder: Strings.ChooseWorkItemQuery });
            if (query) {
                ...
                const workItem: BaseQuickPickItem | undefined = await window.showQuickPick(this.getMyWorkItems(this._serverContext.RepoInfo!.TeamProject, query.wiql!), { matchOnDescription: true, placeHolder: Strings.ChooseWorkItem });
                if (workItem) {
    ...
    public async ChooseWorkItems(): Promise<string[]> {
        ...
        try {
            ...
            const workItem: BaseQuickPickItem | undefined = await window.showQuickPick(this.getMyWorkItems(this._serverContext.RepoInfo!.TeamProject, query), { matchOnDescription: true, placeHolder: Strings.ChooseWorkItem });
            if (workItem) {
    ...
    private async showWorkItems(wiql: string): Promise<void> {
        ...
        const workItem: BaseQuickPickItem | undefined = await window.showQuickPick(this.getMyWorkItems(this._serverContext.RepoInfo!.TeamProject, wiql), { matchOnDescription: true, placeHolder: Strings.ChooseWorkItem });
        if (workItem) {
            let url: string | undefined = undefined;
            if (workItem.id === undefined) {
                Telemetry.SendEvent(TelemetryEvents.OpenAdditionalQueryResults);
    ...
    public async GetPinnedQueryResultCount(): Promise<number | undefined> {
        try {
    ...
    private getUserName(context: TeamServerContext): string | undefined {
